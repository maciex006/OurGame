...
def create_box ( DIM_X , DIM_Y , POINTS , chamber, matrix ):
...
def load_box( DIM_X , DIM_Y , map_input_path ):
    with open(map_input_path, 'r') as map_input:
        ...
        for i in range(0,len(map_output),10):
            print i
            chamber = map_output[i]
            a = (map_output[i+2], map_output[i+3])
            b = (map_output[i+4], map_output[i+5])
            c = (map_output[i+6], map_output[i+7])
            d = (map_output[i+8], map_output[i+9])
            points = (a, b, c, d)
            matrix = create_box(DIM_X, DIM_Y, points, chamber, matrix)
